...
void MainWindow::upPressed(bool secondPlayer)
{
    ...
    case ZXINTERFACE2_IF: if (secondPlayer) bus->key_press(3, 11); else bus->key_press(1, 12); break;
    }
}

void MainWindow::downPressed(bool secondPlayer)
{
    switch (ui->cbJoystickInterface->currentIndex()) {
    case CURSOR_IF: bus->key_press(4, 12);break;
    case KEMPSTON_IF: bus->kj_button_press(PORT1F::KJ_DOWN);break;
    case ZXINTERFACE2_IF: if (secondPlayer) bus->key_press(2, 11); else bus->key_press(2, 12); break;
    }
}

void MainWindow::leftPressed(bool secondPlayer)
{
    switch (ui->cbJoystickInterface->currentIndex()) {
    case CURSOR_IF: bus->key_press(4, 11);break;
    case KEMPSTON_IF: bus->kj_button_press(PORT1F::KJ_LEFT);break;
    case ZXINTERFACE2_IF: if (secondPlayer) bus->key_press(0, 11); else bus->key_press(4, 12);break;
    }
}

void MainWindow::rightPressed(bool secondPlayer)
{
    switch (ui->cbJoystickInterface->currentIndex()) {
    case CURSOR_IF: bus->key_press(2, 12);break;
    case KEMPSTON_IF: bus->kj_button_press(PORT1F::KJ_RIGHT);break;
    case ZXINTERFACE2_IF: if (secondPlayer) bus->key_press(1, 11); else bus->key_press(3, 12); break;
    }
}

void MainWindow::firePressed(bool secondPlayer)
{
    switch (ui->cbJoystickInterface->currentIndex()) {
    case CURSOR_IF: bus->key_press(0, 12);break;
    case KEMPSTON_IF: bus->kj_button_press(PORT1F::KJ_FIRE);break;
    case ZXINTERFACE2_IF: if (secondPlayer) bus->key_press(4, 11); else bus->key_press(0, 12);break;
    }
}
...
void MainWindow::upReleased(bool secondPlayer)
{
    switch (ui->cbJoystickInterface->currentIndex()) {
    case CURSOR_IF: bus->key_release(3, 12);break;
    case KEMPSTON_IF: bus->kj_button_release(PORT1F::KJ_UP);break;
    case ZXINTERFACE2_IF: if (secondPlayer) bus->key_release(3, 11); else bus->key_release(1, 12); break;
    }

}

void MainWindow::downReleased(bool secondPlayer)
{
    switch (ui->cbJoystickInterface->currentIndex()) {
    case CURSOR_IF: bus->key_release(4, 12);break;
    case KEMPSTON_IF: bus->kj_button_release(PORT1F::KJ_DOWN);break;
    case ZXINTERFACE2_IF: if (secondPlayer) bus->key_release(2, 11); else bus->key_release(2, 12); break;
    }
}

void MainWindow::leftReleased(bool secondPlayer)
{
    switch (ui->cbJoystickInterface->currentIndex()) {
    case CURSOR_IF: bus->key_release(4, 11);break;
    case KEMPSTON_IF: bus->kj_button_release(PORT1F::KJ_LEFT);break;
    case ZXINTERFACE2_IF: if (secondPlayer) bus->key_release(0, 11); else bus->key_release(4, 12); break;
    }
}

void MainWindow::rightReleased(bool secondPlayer)
{
    switch (ui->cbJoystickInterface->currentIndex()) {
    case CURSOR_IF: bus->key_release(2, 12);break;
    case KEMPSTON_IF: bus->kj_button_release(PORT1F::KJ_RIGHT);break;
    case ZXINTERFACE2_IF: if (secondPlayer) bus->key_release(1, 11); else bus->key_release(3, 12); break;
    }
}

void MainWindow::fireReleased(bool secondPlayer)
{
    switch (ui->cbJoystickInterface->currentIndex()) {
    case CURSOR_IF: bus->key_release(0, 12);break;
    case KEMPSTON_IF: bus->kj_button_release(PORT1F::KJ_FIRE);break;
    case ZXINTERFACE2_IF: if (secondPlayer) bus->key_release(4, 11); else bus->key_release(0, 12); break;
    }
}
...
//    }
    if (event->type() == QEvent::KeyPress) {
        ...
        case W_SCANCODE:    if (ui->cbJoystickInterface->currentIndex() == ZXINTERFACE2_IF) upPressed(true); else keyPressed(sc);break;
        case S_SCANCODE:    if (ui->cbJoystickInterface->currentIndex() == ZXINTERFACE2_IF) downPressed(true); else keyPressed(sc);break;
        case A_SCANCODE:    if (ui->cbJoystickInterface->currentIndex() == ZXINTERFACE2_IF) leftPressed(true); else keyPressed(sc);break;
        case D_SCANCODE:    if (ui->cbJoystickInterface->currentIndex() == ZXINTERFACE2_IF) rightPressed(true); else keyPressed(sc);break;
        case SPACE_SCANCODE:if (ui->cbJoystickInterface->currentIndex() == ZXINTERFACE2_IF) firePressed(true); else keyPressed(sc);break;
...
//        qDebug() << "Key released: "  << ke->nativeScanCode();
        ...
        case W_SCANCODE:    if (ui->cbJoystickInterface->currentIndex() == ZXINTERFACE2_IF) upReleased(true); else keyReleased(sc);break;
        case S_SCANCODE:    if (ui->cbJoystickInterface->currentIndex() == ZXINTERFACE2_IF) downReleased(true); else keyReleased(sc);break;
        case A_SCANCODE:    if (ui->cbJoystickInterface->currentIndex() == ZXINTERFACE2_IF) leftReleased(true); else keyReleased(sc);break;
        case D_SCANCODE:    if (ui->cbJoystickInterface->currentIndex() == ZXINTERFACE2_IF) rightReleased(true); else keyReleased(sc);break;
        case SPACE_SCANCODE:if (ui->cbJoystickInterface->currentIndex() == ZXINTERFACE2_IF) fireReleased(true); else keyReleased(sc);break;
...
void MainWindow::on_cbPal_currentIndexChanged(int index)
{

    if (index == 0)
        ui->screen->setIndexPlus(0);
    else if (index == 1)
        ui->screen->setIndexPlus(16);
}

void MainWindow::on_action_Load_a_screenshot_triggered()
{
    QString fileName = QFileDialog::getOpenFileName(this, "Choose file", "", "*.scr");
...
{
    QString fileName = QFileDialog::getSaveFileName(this, "Select path and filename", "", "*.scr");
...
{
    QMessageBox::about(this, "About", "This is emulator ZX SPECTRUM created by unyuu and updated by shadowarri");
}

void MainWindow::on_actionLoad_a_snapshot_triggered()
{
    QString fileName = QFileDialog::getOpenFileName(this, "Choose file", "", "*.z80");
    load_z80(fileName);
}

void MainWindow::on_actionSave_a_snapshot_triggered()
{
    QString fileName = QFileDialog::getSaveFileName(this, "Select path and filename", "", "*.z80");
    save_z80(fileName);
}
